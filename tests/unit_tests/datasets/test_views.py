...
class DatasetUpdateViewTests(TestCase):
    ...
    def test_dataset_update_function_resolves(self):
        request = HttpRequest()
        response = dataset_update(request, account_slug=self.a1.account_slug,
            dataset_slug=self.ds1.dataset_slug, pk=self.ds1.pk)
        self.assertEqual(response.status_code, 200)
    ...
    def test_dataset_update_view_title_is_correct(self):
        response = self.client.get(reverse("datasets:dataset_update",
            kwargs={"account_slug": self.a1.account_slug,
            "dataset_slug": self.ds1.dataset_slug, "pk": self.ds1.pk}))
        self.assertIn("<title>ZMT | Update {title}</title>".format(title=self.ds1.title),
            response.content.decode("utf-8"))

    def test_dataset_update_view_redirects_to_dataset_detail_view_on_save(self):
        response = self.client.post(reverse("datasets:dataset_update",
            kwargs={"account_slug": self.a1.account_slug,
            "dataset_slug": self.ds1.dataset_slug, "pk": self.ds1.pk}),
            data={"author": "pat", "title": "test dataset",
                "description": "This is a test dataset",
        ...
        self.assertEqual(response["location"],
             "/test_user/test-dataset/{pk}/".format(pk=self.ds1.pk))
