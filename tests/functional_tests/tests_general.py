...
class NewVisitorTest(BaseLiveTest):
    ...
    def test_can_find_correct_web_page(self):
        ...
        for browser in self.browsers:
            ...
            self.assertIn('ZMT', browser.title)
...
class DatasetFormPageTests(BaseLiveTest):
    ...
    def test_can_use_dataset_create_form_to_make_new_dataset(self):
        ...
        for browser in self.browsers:
            ...
            # Get form inputs
            author_input =  browser.find_element_by_id('id_author')
            title_input = browser.find_element_by_id('id_title')
            description_input = browser.find_element_by_id('id_description')
            url_input = browser.find_element_by_id('id_url')
    ...
    def test_can_update_existing_dataset(self):
        for browser in self.browsers:
            ...
            description_input = browser.find_element_by_id('id_description')
            url_input = browser.find_element_by_id('id_url')
            public_access_input = browser.find_element_by_id('id_public_access')
            submit_button = browser.find_element_by_id('submit_dataset')
    ...
    def test_can_remove_dataset(self):
        for browser in self.browsers:
            ...
            remove_button = browser.find_element_by_id('confirm_remove_button')
            remove_button.click()
    ...
    def test_can_change_dataset_from_NOT_password_protected_to_password_protected(self):
        for browser in self.browsers:
            ...
            public_access_input = browser.find_element_by_id('id_public_access')
            url_input = browser.find_element_by_id('id_url')
            dataset_user_input = browser.find_element_by_id('id_dataset_user')
            dataset_password_input = browser.find_element_by_id('id_dataset_password')
            submit_button = browser.find_element_by_id('submit_dataset')
